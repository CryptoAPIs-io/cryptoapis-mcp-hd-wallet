...
const CRYPTOAPIS_SERVER_INFO = {
    name: "cryptoapis-hd-wallet",
    version: "0.1.0",
    title: "CryptoAPIs HD Wallet",
...
export async function startHdWalletServer(opts: StartOptions) {
    ...
    if (opts.transport === "stdio") {
        ...
        console.error("cryptoapis-hd-wallet MCP running (stdio)");
        return;
    }
    ...
    const transport = new StreamableHTTPServerTransport({ sessionIdGenerator: stateless ? undefined : () => randomUUID() });
    ...
    app.listen(port, host, () => {
        console.error(`cryptoapis-hd-wallet MCP running (http) at http://${host}:${port}${path}`);
        if (cfg.apiKey) {
